...
#ifndef MLS_SQUAREMATRIX_H
#define MLS_SQUAREMATRIX_H

#include"mlsPointTpl.h"
...
namespace mlsCore
{
	template <typename Type> class SquareMatrixTpl
	{
	public:
		...
		SquareMatrixTpl(const float M16f[], bool rotationOnly = false)
		{
			...
			if (init(size))
			{
				for (unsigned r = 0; r < size; r++)
					...
					{
						m_values[r][c] = static_cast<Type>(M16f[c * 4 + r]);
					}
			}
		}
		...
		SquareMatrixTpl(const double M16d[], bool rotationOnly = false)
		{
			...
			if (init(size))
			{
				for (unsigned r = 0; r < size; r++)
				{
					for (unsigned c = 0; c < size; c++)
					{
						m_values[r][c] = static_cast<Type>(M16d[c * 4 + r]);
					}
				}
			}
		}
		...
		**/
		...
		inline Type* row(unsigned index) { return m_values[index]; }

		//! Sets a particular matrix value
		void inline setValue(unsigned row, unsigned column, Type value)
		{
			m_values[row][column] = value;
		}

		//! Returns a particular matrix value
		Type inline getValue(unsigned row, unsigned column) const
		{
		...
		**/
		void apply(const Type Vec[], Type result[]) const
		{
			for (unsigned r = 0; r < m_matrixSize; r++)
			{
				Type sum = 0;
				for (unsigned k = 0; k < m_matrixSize; k++)
					sum += m_values[r][k] * static_cast<Type>(Vec[k]);
				result[r] = sum;
			}
		}
		...
		void clear()
		{
			for (unsigned r = 0; r < m_matrixSize; ++r)
			{
				memset(m_values[r], 0, sizeof(Type)*m_matrixSize);
			}
		}
		...
		SquareMatrixTpl inv() const
		{
			...
			Type** tempM = nullptr;
			{
				...
				{
					tempM[i] = new Type[2 * m_matrixSize];
					if (!tempM[i])
					{
						//not enough memory
		...
		**/
		void initFromQuaternion(const double q[])
		{
			...
			m_values[0][0] = static_cast<Type>(q00 + q11 - q22 - q33);
			m_values[1][1] = static_cast<Type>(q00 - q11 + q22 - q33);
			m_values[2][2] = static_cast<Type>(q00 - q11 - q22 + q33);
			m_values[0][1] = static_cast<Type>(2.0*(q12 - q03));
			m_values[1][0] = static_cast<Type>(2.0*(q12 + q03));
			m_values[0][2] = static_cast<Type>(2.0*(q13 + q02));
			m_values[2][0] = static_cast<Type>(2.0*(q13 - q02));
			m_values[1][2] = static_cast<Type>(2.0*(q23 - q01));
			m_values[2][1] = static_cast<Type>(2.0*(q23 + q01));
		}
		...
		**/
		...
		Type deltaDeterminant(unsigned column, Type* Vec) const
		{
			...
			for (unsigned i = 0; i < m_matrixSize; i++)
			{
				if (column == i)
				{
					for (unsigned j = 0; j < m_matrixSize; j++)
					{
						mat.m_values[j][i] = static_cast<Type>(*Vec);
						Vec++;
					}
				}
				else
	...
	private:
		...
		**/
		bool init(unsigned size)
		{
			...
			m_values = new Type*[m_matrixSize] {};
			m_pData = new Type[matrixSquareSize]{};

			if ((m_values == nullptr) || (m_pData == nullptr))
			{
				return false;
			}
			...
			for (unsigned i = 0; i < m_matrixSize; i++)
			{
				m_values[i] = m_pData + (i * m_matrixSize);
			}
		...
		double computeSubDet(Type** mat, unsigned matSize) const
		{
			if (matSize == 2)
			{
			...
			Type** subMat = new Type*[matSize - 1];
			if (subMat)
			{
				double subDet = 0;
				double sign = 1.0;
